...
//            __rsHTML += "<td><h5>" + String.format("%,.2f", Float.parseFloat(__rsData1.getString("total_amount"))) + "</h5></td>";
    ...
    private JSONObject _updateEvetnQTY(Connection conn, JSONObject param) throws SQLException, Exception {
        ...
        String __strLotNumber = !param.isNull("lot_number") && !param.getString("lot_number").trim().isEmpty() ? param.getString("lot_number") : "";

        String __strQUERY = "UPDATE pp_trans_detail SET event_qty='" + __strEventQTY + "',remark='" + __strRemark + "',status='" + __strStatus + "' WHERE doc_no = '" + __strDocNo + "' AND ic_code='" + __strIcCode + "'  AND lot_number_1 = '" + __strLotNumber + "' ";
